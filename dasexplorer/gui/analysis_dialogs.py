...
class FKViewDialog(QtWidgets.QDialog):
    """
    Show the F-K filtered waterfall for a BBox/OBBox annotation region.

    Recalculates the F-K filter on the cropped annotation region with a
    configurable margin. Parameters default to those in the main panel but
    can be adjusted within the dialog.
    """

    def __init__(self, ann, dataset, colormap: pg.ColorMap,
                 c_min: float = 1400.0, c_max: float = 3500.0,
                 fk_fmin: float = 1.0, fk_fmax: float = 50.0,
                 vmin: float = None, vmax: float = None,
                 tr_bandpass: np.ndarray = None, parent=None):
        super().__init__(parent)
        self.setWindowTitle(
            f"FK View — Event [{ann.id}]  "
            f"t={ann.t0:.2f}–{ann.t1:.2f} s  "
            f"d={ann.d0:.0f}–{ann.d1:.0f} m"
        )
        self.setWindowFlags(
        ...
        screen = QtWidgets.QApplication.primaryScreen().availableGeometry()
        self.resize(int(screen.width() * 0.60), int(screen.height() * 0.60))

        self.ann         = ann
        self.dataset     = dataset
        self.cmap        = colormap
        self._tr_bandpass = tr_bandpass   # bandpass array from main waterfall
        self._ext_vmin   = vmin
        self._ext_vmax   = vmax

        # View region: annotation + 50% margin
        _mt = (ann.t1 - ann.t0) * 0.5
        _md = (ann.d1 - ann.d0) * 0.5
        self._t0_view = max(float(dataset.time_s[0]),  ann.t0 - _mt)
        self._t1_view = min(float(dataset.time_s[-1]), ann.t1 + _mt)
        self._d0_view = max(float(dataset.dist_m[0]),  ann.d0 - _md)
        self._d1_view = min(float(dataset.dist_m[-1]), ann.d1 + _md)

        # FK parameters (mutable)
        self._c_min   = c_min
        self._c_max   = c_max
        self._fk_fmin = fk_fmin
        self._fk_fmax = fk_fmax
    ...
    def _build_ui(self):
        ...
        def _spinbox(val, lo, hi, step, decimals=1):
            sb = QtWidgets.QDoubleSpinBox()
            sb.setRange(lo, hi)
            sb.setDecimals(decimals)
        ...
        ctrl.addWidget(QtWidgets.QLabel("c [m/s]:"))
        self.spin_cmin = _spinbox(self._c_min, 0, 1e7, 100)
        ctrl.addWidget(self.spin_cmin)
        ctrl.addWidget(QtWidgets.QLabel("–"))
        self.spin_cmax = _spinbox(self._c_max, 0, 1e7, 100)
        ctrl.addWidget(self.spin_cmax)

        ctrl.addSpacing(12)
        ctrl.addWidget(QtWidgets.QLabel("f [Hz]:"))
        self.spin_fmin = _spinbox(self._fk_fmin, 0, 1e4, 0.5)
        ctrl.addWidget(self.spin_fmin)
        ctrl.addWidget(QtWidgets.QLabel("–"))
        self.spin_fmax = _spinbox(self._fk_fmax, 0, 1e4, 0.5)
        ctrl.addWidget(self.spin_fmax)

        ctrl.addSpacing(12)
        self.chk_env = QtWidgets.QCheckBox("Hilbert Envelope")
    ...
    def _compute_and_render(self):
        ds     = self.dataset
        tr_src = self._tr_bandpass if self._tr_bandpass is not None else ds.tr

        # Ensure shape matches dataset
        if tr_src.shape[0] != ds.n_dist:
            tr_src = ds.tr

        ti0 = max(0, int(np.argmin(np.abs(ds.time_s - self._t0_view))))
        ti1 = min(ds.n_time, int(np.argmin(np.abs(ds.time_s - self._t1_view))) + 1)
        di0 = max(0, int(np.argmin(np.abs(ds.dist_m - self._d0_view))))
        di1 = min(ds.n_dist, int(np.argmin(np.abs(ds.dist_m - self._d1_view))) + 1)

        if ti1 <= ti0 or di1 <= di0:
            print("[FKViewDialog] empty crop region")
            return

        tr_crop = tr_src[di0:di1, ti0:ti1].astype(np.float32)

        # Apply FK filter on cropped region
        try:
            ...
            tr_fk = fk_filter_apply(tr_crop, fk, tapering=False)
        except Exception as e:
            tr_fk = tr_crop
            print(f"[FKViewDialog] FK error: {e}")
        ...
        if self._envelope:
            ...
            n    = tr_fk.shape[1]
            nfft = _nfl(n)
            tr_fk = np.abs(_sp.hilbert(tr_fk, N=nfft, axis=1)[:, :n]).astype(np.float32)

        tr_fk = np.asarray(tr_fk, dtype=np.float32)
        ...
        p99  = float(np.percentile(np.abs(tr_fk), 99))
        ...
        self.image_item.sigImageChanged.disconnect(self.histogram.item.imageChanged)
        self.image_item.setImage(tr_fk, autoLevels=False, levels=[vmin, vmax])
        self.image_item.setRect(QtCore.QRectF(x0, y0, x1 - x0, y1 - y0))
        self.image_item.sigImageChanged.connect(self.histogram.item.imageChanged)
        self.histogram.item.imageChanged(autoLevel=False)
        self.histogram.setLevels(vmin, vmax)
    ...
    def _on_apply(self):
        self._c_min    = self.spin_cmin.value()
        self._c_max    = self.spin_cmax.value()
        self._fk_fmin  = self.spin_fmin.value()
        self._fk_fmax  = self.spin_fmax.value()
        self._envelope = self.chk_env.isChecked()
        self._compute_and_render()

    def _export_png(self):
        path, _ = QtWidgets.QFileDialog.getSaveFileName(
            self, "Export FK View", f"fkview_{self.ann.id}.png", "PNG (*.png)"
        )
        if not path:
            return
...
class RGBViewDialog(QtWidgets.QDialog):
    """
    Show the multispectral RGB composite for a BBox/OBBox annotation region.

    Computes the RGB representation on the cropped annotation region (raw
    unfiltered array). Band parameters and percentile default to those in
    the main panel but can be adjusted within the dialog.
    """

    def __init__(self, ann, dataset,
                 r_min=1.0, r_max=5.0,
                 g_min=5.0, g_max=15.0,
                 b_min=15.0, b_max=40.0,
                 percentile=90.0,
                 tr_src: np.ndarray = None, parent=None):
        super().__init__(parent)
        self.setWindowTitle(
            f"RGB View — Event [{ann.id}]  "
            f"t={ann.t0:.2f}–{ann.t1:.2f} s  "
            f"d={ann.d0:.0f}–{ann.d1:.0f} m"
        )
        self.setWindowFlags(
        ...
        self.ann     = ann
        self.dataset = dataset
        # Always use the raw dataset array — RGB filters from scratch per band
        self._tr_src = dataset.tr if tr_src is None else tr_src
        ...
        # Band parameters
        self._r_min = r_min; self._r_max = r_max
        self._g_min = g_min; self._g_max = g_max
        self._b_min = b_min; self._b_max = b_max
    ...
    def _build_ui(self):
        ...
        # ── Control bar (single horizontal row) ───────────────────────
        ctrl = QtWidgets.QHBoxLayout()
        ctrl.setSpacing(6)

        def _sb(val, decimals=1, step=0.5, width=72):
            sb = QtWidgets.QDoubleSpinBox()
            sb.setRange(0, 1e4)
            sb.setDecimals(decimals)
            sb.setSingleStep(step)
        ...
        ctrl.addWidget(QtWidgets.QLabel("R:"))
        self.spin_rmin = _sb(self._r_min); ctrl.addWidget(self.spin_rmin)
        ctrl.addWidget(QtWidgets.QLabel("–"))
        self.spin_rmax = _sb(self._r_max); ctrl.addWidget(self.spin_rmax)

        ctrl.addSpacing(8)
        ctrl.addWidget(QtWidgets.QLabel("G:"))
        self.spin_gmin = _sb(self._g_min); ctrl.addWidget(self.spin_gmin)
        ctrl.addWidget(QtWidgets.QLabel("–"))
        self.spin_gmax = _sb(self._g_max); ctrl.addWidget(self.spin_gmax)

        ctrl.addSpacing(8)
        ctrl.addWidget(QtWidgets.QLabel("B:"))
        self.spin_bmin = _sb(self._b_min); ctrl.addWidget(self.spin_bmin)
        ctrl.addWidget(QtWidgets.QLabel("–"))
        self.spin_bmax = _sb(self._b_max); ctrl.addWidget(self.spin_bmax)

        ctrl.addSpacing(8)
        ctrl.addWidget(QtWidgets.QLabel("Pct:"))
        self.spin_pct = _sb(self._pct, decimals=1, step=1.0, width=60)
    ...
    def _compute_and_render(self):
        ds  = self.dataset
        tr  = self._tr_src   # raw unfiltered array, shape (n_dist, n_time)

        # Ensure tr matches dataset spatial dimension
        if tr.shape[0] != ds.n_dist:
            print(f"[RGBViewDialog] tr shape mismatch: {tr.shape[0]} vs {ds.n_dist}")
            tr = ds.tr
        ...
        if ti1 <= ti0 or di1 <= di0:
            print("[RGBViewDialog] empty crop region")
            return
        ...
        try:
            ...
            rgb = compute_rgb_composite(
                tr_crop, ds.fs_hz,
                r_band=(self._r_min, self._r_max),
                g_band=(self._g_min, self._g_max),
                b_band=(self._b_min, self._b_max),
                percentile=self._pct,
            )
        except Exception as e:
            print(f"[RGBViewDialog] RGB error: {e}")
            return
        ...
        # pyqtgraph ImageItem with RGB array needs levels=None
    ...
    def _on_apply(self):
        self._r_min = self.spin_rmin.value(); self._r_max = self.spin_rmax.value()
        self._g_min = self.spin_gmin.value(); self._g_max = self.spin_gmax.value()
        self._b_min = self.spin_bmin.value(); self._b_max = self.spin_bmax.value()
        self._pct   = self.spin_pct.value()
        self._compute_and_render()
